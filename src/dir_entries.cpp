...
namespace tree
{
	...
	directory_entries::directory_entries(std::string path, bool with_files) {
		this->set_path(path, with_files);
	}

	directory_entries::directory_entries(std::filesystem::path path, bool with_files) {
		this->set_path(path, with_files);
	}
	...
	inline std::vector<std::filesystem::directory_entry> directory_entries::get_entries() const noexcept {
		return entries;
	}

	inline bool directory_entries::is_directory_empty() const noexcept {
		return entries.size() == 0;
	}

	inline bool directory_entries::is_path_exist() const noexcept {
		return std::filesystem::exists(this->path);
	}

	inline void directory_entries::set_path(std::string path, bool with_files) {
